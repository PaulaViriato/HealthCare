...
$all_operadora = $DAOOperadora->select_all();
...
if (isset($_POST["permitexec"])){
	if (strcasecmp ("".$_POST["permitexec"], "true") == 0){
		if (intval($_POST["typeoperation"]) == 0){
			if ((isset($_POST["codprest"]))&&(isset($_POST["id_operadora"]))){
				...
				$newprestadora->get_OPERADORA()->set_id ($_POST["id_operadora"]);
		...
		if (intval($_POST["typeoperation"]) == 1){
			if ((isset($_POST["idcnes"]))&&(isset($_POST["idoperadora"]))){
				$array_prest = $DAOPrestadora->where_order_clause(2, "id_cnes = '".$_POST["idcnes"]."' AND id_operadora = ".$_POST["idoperadora"]);
...
<body>
			...
			<div class="sidebar-scroll">
				<nav>
					<ul class="nav">
						...
						<li><a href="prestadores.php" class="active"><i class="lnr lnr-users"></i> <span>Prestadores Operando</span></a></li>
						...
						<div class="col-md-12">
							<div class="panel">
								<div class="panel-heading">
									<h3 class="panel-title">Prestadores de Serviços por Operadora</h3>
								</div>
								<div id="search-panel">
									...
									<div class="panel-body" style="width:100%;padding-top:0px;padding-bottom:10px;">
										<div class="btn-group" style="margin-right:1%;width:20%;">
											...
											<ul class="dropdown-menu" role="menu" aria-labelledby="dropdownMenu">
												<li><a tabindex="0" onclick="modifyDropdown('0')">Operadora</a></li>
												<li><a tabindex="1" onclick="modifyDropdown('1')">Registro</a></li>
												<li><a tabindex="2" onclick="modifyDropdown('2')">Nome Fantasia</a></li>
												<li><a tabindex="3" onclick="modifyDropdown('3')">CNPJ ou CPF</a></li>
												<li><a tabindex="4" onclick="modifyDropdown('4')">Município</a></li>
												<li><a tabindex="5" onclick="modifyDropdown('5')">UF</a></li>
												<li><a tabindex="6" onclick="modifyDropdown('6')">Todos</a></li>
											</ul>
										</div>
		...
		<div class="modal-dialog">
			<div class="modal-content">
					<div class="modal-header">
						...
						<h4 class="modal-title">Cadastrar Prestador de Serviços em Operadora </h4>
					</div>
						...
						<div class="modal-body">
							<div class="row">
								<div>
									...
									<input id="id_operadora" type="hidden" name="id_operadora" required/>
								...
								<div id="cadastro_prest" class="collapse">
									...
									<div id="div-operadora" class="col-md-12" style="margin-bottom: 10px;">
										<div>
											<span> Nome da Operadora</span>
											<button class="btn dropdown-toggle" id="selectoperadora" data-toggle="dropdown" style="background-color:white;border-color:#A4A4A4;width:100%;height:27px;padding-top:2px;">
												<span id="textoperadora">Selecione Operadora</span>
												<span class="caret"></span>
											</button>
											<ul class="dropdown-menu" role="menu" aria-labelledby="dropdownMenu" style="background-color:white;
											width:95%;height:27px;padding-top:0px;margin-top:0px;margin-left:15px;margin-right:15px;border-width:0px;">
												<?php
													$count = 1;
													foreach ($all_operadora as $operadora){
														echo '<li style="background-color:white;border-style:solid;border-width:1px;border-color:#A4A4A4; width:100%;height:27px;padding-top:2px;margin-top:0px;">';
														echo '<a style="color:black;padding-top:1px;" tabindex="'.$count.'" 
														onclick="modifyDropdownOpe(\''.$operadora->get_nome().' ['.$operadora->get_login().']['.$operadora->get_id().']\', \''.$operadora->get_id().'\')"
														>'.$operadora->get_nome().' ['.$operadora->get_login().']['.$operadora->get_id().']</a></li>';
														$count = $count + 1;
													}
												?>
											</ul>
			...
			<div class="modal-content">
				<form enctype="multipart/form-data" autocomplete="off" method="POST" action="prestadores.php">
					...
					<div class="modal-body">
						...
						<input id="idoperadoraexcl" type="hidden" name="idoperadora" required/>
	...
	<script type="text/javascript">
		...
		function modifyDropdown (index){
			...
			if (parseInt(index) == 0){ $('#strcampo').html ("Operadora &nbsp<span class=\"caret\"></span>"); }
			if (parseInt(index) == 1){ $('#strcampo').html ("Registro &nbsp<span class=\"caret\"></span>"); }
			if (parseInt(index) == 2){ $('#strcampo').html ("Nome Fantasia &nbsp<span class=\"caret\"></span>"); }
			if (parseInt(index) == 3){ $('#strcampo').html ("CNPJ ou CPF &nbsp<span class=\"caret\"></span>"); }
			if (parseInt(index) == 4){ $('#strcampo').html ("Município &nbsp<span class=\"caret\"></span>"); }
			if (parseInt(index) == 5){ $('#strcampo').html ("UF &nbsp<span class=\"caret\"></span>"); }
			if (parseInt(index) == 6){
		...
		function search (){
			if (((parseInt(document.getElementById("searchindex").value) == -1)||
				(document.getElementById("searchtxt").value == ""))&&(parseInt(document.getElementById("searchindex").value) != 6)){
			...
			if (index == 1){ text = "UPPER(id) LIKE '%"+txtsc.toUpperCase()+"%'"; }
			if (index == 2){ text = "UPPER(no_fantasia) LIKE '%"+txtsc.toUpperCase()+"%'"; }
			if (index == 3){
				text = "UPPER(cnpj) LIKE '%"+txtsc.toUpperCase()+"%' OR ";
				text = text+"UPPER(cpf) LIKE '%"+txtsc.toUpperCase()+"%'";
			}
			if (index == 4){ text = "UPPER(nm_municipio) LIKE '%"+txtsc.toUpperCase()+"%'"; }
			if (index == 5){ text = "UPPER(uf) LIKE '%"+searchUF(txtsc)+"%'"; }
			if (index == 6){ text = "id IS NOT NULL"; }

			txtwhereclauseope = null;
			txtwhereclause = null;
			if ((index == 0)||(index == 6)){ txtwhereclauseope = text; }
			else { txtwhereclause = text; }
		...
		function modifyDropdownOpe (text, id){
			document.getElementById("id_operadora").value = id;
			$('#textoperadora').text(""+text);
		}

		function excluirOperadora(id_cnes, id_operadora){
			document.getElementById("idcnesexcl").value = id_cnes;
			document.getElementById("idoperadoraexcl").value = id_operadora;
		...
		function desbloqCadast(){
			...
			var value = searchUF(""+document.getElementById("auto-uf").value);
