	...
	if (strcasecmp ("".$_POST["permitexec"], "true") == 0){
		if (intval($_POST["typeoperation"]) == 0){
			if (isset($_POST["ap-novasenha"])){
				$administrador = $DAOAdministrador->select_id($_SESSION["healthcare-user"]["id"]);
				$administrador->set_senha($_POST["ap-novasenha"]);
				$result = $DAOAdministrador->update($administrador);
				if (!$result){
					$hasMessage = true;
	...
	<script type="text/javascript">
		...
		$(document).ready(function() {
			var table = $('#table-materiais').DataTable({
				"language": {
					"lengthMenu": "Mostrar &nbsp _MENU_ &nbsp funcionários por página",
