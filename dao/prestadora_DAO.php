...
class Prestadora_DAO {
	...
	public function get_insert ($objeto_recebido, $whole = true, $intro = false, $instance = false){
		$sql = "";
		if ($whole == true){
			$sql .= "INSERT INTO tb_prestadora (id_cnes, id_operadora) VALUES (
			...
			if ($intro == true){ $sql .= "INSERT INTO tb_prestadora (id_cnes, id_operadora) VALUES "; }
			if ($instance == true){
				$sql .= "(".(is_null($objeto_recebido->get_CNES()->get_id()) ? "null" : "'".$objeto_recebido->get_CNES()->get_id()."'").", 
				".(is_null($objeto_recebido->get_OPERADORA()->get_id()) ? "null" : $objeto_recebido->get_OPERADORA()->get_id()).")";
			}
		}
		return $sql;
	...
	public function insert ($objeto_recebido, $database = null){
		...
		$id = -1;
		if ($query_result){
			$sql = "SELECT id FROM tb_prestadora WHERE 
			".(is_null($objeto_recebido->get_CNES()->get_id()) ? "isnull(id_cnes)" : "id_cnes = '".$objeto_recebido->get_CNES()->get_id()."'")." AND
	...
	public function unmodified ($objeto_recebido, $database = null){
		...
		$sql = "SELECT COUNT(id) AS count FROM tb_prestadora WHERE 
	...
	public function delete ($id, $database = null){
		...
		$tabelas = $DAOTabela->select_by_Prestadora($id, $dbh);
		foreach ($tabelas as $tabela){ $DAOTabela->delete($tabela->get_id(), $dbh); }

		$prestadora = $this->select_id($id, $dbh);
		$DAOLogin = new Login_DAO();
		$DAOLogin->delete_prestadora($prestadora->get_CNES()->get_id(), $dbh);

		$sql = "DELETE FROM tb_prestadora WHERE id = ".$id.";";
		$query_result = $dbh->query ($sql);
	...
	public function where_order_clause ($type, $wherecl = null, $ordercl = null, $objeto_recebido = null, $database = null){
		...
		if ($type == 1){
			if (is_null($wherecl) == false){
				$sql = "DELETE FROM tb_prestadora WHERE ".$wherecl.";";
				$resreturn = $dbh->query ($sql);
			}
		}
	...
	public function exist_id ($id, $database = null){
		$dbh = $database;
		if (is_null ($database)){ $dbh = getDbh(); }
		$sql = "SELECT COUNT(id) AS qtd FROM tb_prestadora WHERE id = ".$id.";";
	...
	public function select_id ($id, $database = null){
		$dbh = $database;
		if (is_null ($database)){ $dbh = getDbh(); }
		$sql = "SELECT * FROM tb_prestadora WHERE id = ".$id.";";
	...
	public function select_all ($database = null, $columns = null){
		...
		if (is_null ($database)){ $dbh = getDbh(); }
		$sql = "SELECT * FROM tb_prestadora;";
		if (!is_null ($columns)){ $sql = "SELECT ".$columns." FROM tb_prestadora;"; }
	...
	public function select_by_CNES ($id, $database = null){
		$dbh = $database;
		if (is_null ($database)){ $dbh = getDbh(); }
		$sql = "SELECT * FROM tb_prestadora WHERE id_cnes = '".$id."';";
	...
	public function select_by_Operadora ($id, $database = null){
		$dbh = $database;
		if (is_null ($database)){ $dbh = getDbh(); }
		$sql = "SELECT * FROM tb_prestadora WHERE id_operadora = ".$id.";";
